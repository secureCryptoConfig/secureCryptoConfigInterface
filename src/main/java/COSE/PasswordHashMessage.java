...
public class PasswordHashMessage extends PasswordHashCommon {

	/**
	 * Create a PasswordHashMessage object. This object corresponds to the encrypt
	 * message format in COSE. The leading CBOR tag will be emitted. The message
	 * content will be emitted.
	 */
	public PasswordHashMessage() {
		this(true, true);
	}

	/**
	 * Create a PasswordHashMessage object. Emission of the leading tag is controlled by
	 * the parameter.
	 * 
	 * @param emitTag emit leading tag when message is serialized
	 */
	public PasswordHashMessage(boolean emitTag) {
		this(emitTag, true);
	}

	/**
	 * Create a PasswordHashMessage object for which the emission of the leading tag and
	 ...
	 * @since COSE 0.9.1
	 */
	public PasswordHashMessage(boolean emitTag, boolean emitContent) {
		this.emitTag = emitTag;
		this.contextString = "PasswordHash";
		this.emitContent = emitContent;
		this.messageTag = MessageTag.PasswordHash;
	}

	/**
	 * Internal function used in creating a PasswordHashMessage object from a byte string.
	 * 
	 * @param messageObject PasswordHashMessage encoded object.
	...
	protected void DecodeFromCBORObject(CBORObject messageObject) throws CoseException {
		...
		if (messageObject.get(3).getType() == CBORType.ByteString)
			rgbHashed = messageObject.get(3).GetByteString();
		else
			throw new CoseException("Invalid Hash structure");

		if (messageObject.get(4).getType() == CBORType.ByteString)
			salt = messageObject.get(4).GetByteString();
		else
			throw new CoseException("Invalid Hash structure");
	...
	/**
	 * Internal function used to create a serialization of a PasswordHashMessage 
	 * 
	...
	public void passwordHash() throws CoseException, IllegalStateException {
		if (rgbContent == null)
		...
		rgbHashed = computePasswordHash(rgbContent);

	}

	public void passwordHashWithSalt(byte[] s) throws CoseException, IllegalStateException {
		salt = s;
		if (rgbContent == null)
			throw new CoseException("No Content Specified");
		if (rgbHashed != null)
		...
		rgbHashed = computePasswordHashWithSalt(rgbContent, s);
